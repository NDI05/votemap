...
exports.searchUser = async (req, res) => {
    try {
        ...
        if (!search) {
            return res.status(400).json({ message: 'Parameter pencarian tidak boleh kosong.' });
        }
        const query = 'CALL sp_users_search(?, ?)';
        const [rows] = await db.execute(query, [actorIdUser, search]);
        res.status(200).json(rows[0]);
    } catch (error) {
        const { statusCode, message } = handleDbError(error);
        return res.status(statusCode).json({ message });
